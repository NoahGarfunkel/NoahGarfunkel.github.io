class BarchartAir {
    ...
    initVis() {
      ...
      vis.data = vis.data.filter(d => d.air_quality !== -1);
    ...
    updateVis() {
      ...
      vis.data.sort((a, b) => a.air_quality - b.air_quality);
    ...
    renderVis() {
      ...
      // Add rectangles
      let bars = vis.chart.selectAll('.bar2')
      ...
      bars
          .on('mouseover', (event,d) => {
            d3.select('#tooltip')
              .style('opacity', 1)
              .html(`<div class="tooltip-label">County</div>${d.display_name}<br><div class="tooltip-label">Air Quality</div>${d.air_quality}`);
          })
          .on('mousemove', (event) => {
            d3.select('#tooltip')
