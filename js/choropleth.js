class ChoroplethMap {
  ...
  initVis() {
    ...
    vis.data.objects.counties.geometries = vis.data.objects.counties.geometries.filter(geometry => {
        return geometry.properties.poverty_perc !== -1;
    });
    ...
    vis.counties = vis.g.append("g")
      ...
      vis.counties
                .on('mousemove', (event, d) => {
                    const povertyPerc = d.properties.poverty_perc ? `<strong>${d.properties.poverty_perc}</strong> poverty percentage` : 'No data available'; 
                    d3.select('#tooltip2')
                      .style('display', 'block')
